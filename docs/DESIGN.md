...
IF UDP 池全部不可用:                          # C) FR13
  自检: 现任线路 API delay 是否正常?           # 窄版 E / NFR10
    正常 → 判为探测目标故障 → 本轮作废（不告警、不切换）
    异常 → 探测 TCP_POOL + TAIL_POOL → 同一评分模型择优 → 切换
           IF 也全部不可用 → 判定本地网络故障 → 保持现状 + 告警

告警层（边沿触发，均带冷却）:
  全线故障 / 恢复正常 / 慢速劣化(FR14) / 速度恢复
```
| 保护 | 规则 |
|---|---|
| 探测自检（NFR10） | 探测全 0 但现任线路 `/delay` 正常 → 判为探测目标故障，本轮作废 |
| 慢速劣化告警（FR14） | 最优线路连续 3 轮 < 10 Mbps → 告警；恢复正常后发恢复告警 |
| 限频 | 切换间隔 ≥ 300s |
...
## 11. 演练开关（故障路径必须可演练，NFR9）

| 开关 | 作用 | 验证目标 |
|---|---|---|
| `GUARD_TEST_ALLDEAD=1` | 强制判定全线故障 | 告警链路 |
| `GUARD_TEST_UDPDEAD=1` | 强制 UDP 全挂（自检生效） | NFR10 探测自检 → 本轮作废 |
| `GUARD_TEST_UDPDEAD=2` | 强制 UDP 全挂并跳过自检 | FR13 兜底池动态择优 |

> 教训：两个严重缺陷（`UnboundLocalError` 崩溃、缩进误伤）都只在故障路径暴露，
> 平时正常运行完全看不出。**故障路径不演练 = 没写**。
